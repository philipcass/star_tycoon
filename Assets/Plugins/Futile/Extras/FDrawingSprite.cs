...
public class FDrawingBorder
{
	...
	public FDrawingBorder(float thickness,Color color, bool gradient):base()
	{
...
public class FDrawingCursor
{
	...
	virtual protected void AddBorders (Vector2 leftPoint, Color leftColor, Vector2 leftDirectionNormalized, Vector2 rightPoint, Color rightColor, Vector2 rightDirectionNormalized) {
		if ((borders!=null)&&(borders.Count>0)) {
			int n=borders.Count;
			for (int i=0;i<n;i++) {
				FDrawingBorder border = borders[i];
	...
	virtual public void PushBorder(float thickness, Color color, bool gradient) {
		...
		borders.Add (new FDrawingBorder(thickness, color, gradient));
	}
...
public class FDrawingSprite : FSprite
{
	...
	{
		Flush ();
		_cursor.PushBorder(thickness, color, gradient);
	}
